...
## W8-卡5（2026-09-27）· Agent SSE 流式轨迹——执行过程实时可见（D2 拍板落地）

**增加了什么功能**
- 后端：`run_agent` 增加可选 `on_event` 事件回调（不传时行为零变化），关键节点
  推送 tool_call / tool_result / final_answer / degraded 四类事件；新增
  `POST /api/agent/chat/stream` SSE 端点（限流与 kb 白名单同款防线，worker
  线程 + 队列桥接同步循环，异常转结构化 error 事件不留悬挂流）。
- 前端：输入区新增 `⚡Agent` 模式开关；开启后提问走多步检索循环，轨迹在
  气泡内逐行实时渲染（"正在检索「…」→ 检索完成带来源摘要 → 整理答案"），
  答案带 [S1] 来源标注与参考文档；未知事件类型原样显示一行兜底——未来加新
  工具/新事件前端零改动。

**解决了什么问题**
- "Agent 在后台干了什么"从黑盒变现场直播——演示与排障都看得到每一步。
- 浏览器真机实测抓出 SSE 解析测试未覆盖的真 bug：gen() 曾 `pop("type")`
  导致 data 载荷无类型、轨迹行全部退化为未知图标（event 行与 data 现已双写
  type，加回归断言钉死）；`runAgentTurn` 曾对普通网络错误静默上抛，现一律
  在轨迹区给出红色说明（断网/后端未启动不再无声卡死）。

**验证**
- 新增 5 测试（回调序列/降级事件/异常转 error/SSE 帧序）；全量 pytest exit=0。
- 浏览器真机三验收：轨迹逐行出现 → 答案带来源渲染（狂徒 vs 幻影对比，
  "晚安焰火"正确归位雷兹大招）→ 掉线时轨迹区错误说明且界面可继续操作。

---

## W8-卡5.2（2026-09-27）· 分岔轮写入方案前端确认卡片（卡 3 移交尾巴闭环）

**增加了什么功能**
- 普通聊天自动分流到 Agent 的轮次，若 Agent 提议写知识库（propose_kb_write），
  回答下方弹出"📝 知识库写入方案 · 需人工确认"卡片：目标库/内容预览/理由 +
  【确认写入】【忽略】；确认时首次输入管理密码（复用管理密码门模式），放行后
  卡片变绿色"已写入知识库（可检索）"状态。⚡Agent 模式同款支持。

**解决了什么问题**
- `agent_turn` 曾把 run_agent 的 pending_proposals 丢弃——普通聊天分岔轮到
  达前端时无卡片素材（安全门不受影响：不确认绝不落库，但用户看不到方案）。
- 现两条链路（/send 同步 + /stream SSE done）与 Agent 直连全部透传。

**验证**
- 4 新测试（agent_turn 透传 / /send 与 /stream 带方案与空列表）；全量 pytest exit=0。
- 浏览器真机全链路：方案卡片 → 密码放行 → 落库 agentkb_prop_b3b3cd6（1 块）
  → 测试文档已删除还原。
...
**新增了什么**
- 两个动作类工具进 Agent：`query_tickets`（工单只读查询：状态过滤/条数夹紧 50/答案摘要）、
  `propose_kb_write`（知识库写入**只生成方案绝不执行**：目标库/内容/理由，随响应
  `pending_proposals` 字段回传前端——同步端点与卡 5 SSE `done` 事件都带）。
- 确认门 `POST /api/agent/kb-write/confirm`：双因子凭据 = `proposal_id`（一次性、
  10 分钟过期、内存 store 重启即失效）+ `X-Admin-Key`（复用 v3.30 管理密码门，
  fail-closed）。拒绝矩阵：伪造 404 / 过期 410 / 重放 409 / 入库失败 502。
- 入库执行全系统唯一入口 `agent/actions.confirm_proposal`：先核销后执行（并发确认
  只有一个能过门），链路与工单回流同款（split_text → add_chunks → 文档清单注册，
  `origin=agent_proposed` 管理页可见可删）。
- 决策系统提示加第 7 条动作工具纪律：propose ≠ 已写入，必须告知用户"等待管理员确认"。

**解决了什么问题**
- "Agent 乱写知识库怎么办"这个企业级安全命题有了标准答案：模型只能提案，人拿钥匙。
  真机五步实测全过——对话生成方案（未确认零落库）→ 错密码 403 → 确认落库
  （`agentkb_prop_a17ea3b`，1 块）→ 重放 409 → 检索 Top1 命中写入文档（score=0.71）。

**验证**
- 新增 `test_agent_actions.py` 17 用例（未确认绝不落库工具层+端到端两层、落库断言、
  伪造/过期/重放/密码错/未配置密码五拒）；全量 pytest 0 失败。
- 真机证据：`W8-AgenticRAG/evidence/卡4-验收证据.md`（演示脚本可复跑）。

---

## W8-卡7（2026-09-26）· 知识库切片标题修复（根治"无主切片"）

**优化了哪些地方**
- `document_service.py` 新增 `_split_by_headings`：切分前按 markdown 标题（#/##/###/####）预分段，
  段内再切，**每块前置所属标题**（段首即标题行时不重复前置）；无标题文本（工单等）走原逻辑零影响。

**解决了什么问题**
- 卡 1 重放实锤的检索底座缺陷：切分把 `## 雷兹 / Raze` 标题留在芮娜切片尾部，雷兹技能表格成了
  不含"雷兹"字样的无主切片——查"雷兹"永远命中借标题者，Workflow 与 Agent 同受其害。
- 修复后（两域索引重建：valorant 287 块 / law 735 块）：BM25 查"雷兹" top1 即正确切片；
  Agent 真跑原题**雷兹四技能全部忠实原文**（晚安焰火=火箭发射器）——「晚安焰火=团队视野」
  幻觉自发现、溯源、治理到治愈全链路闭环。

**验证**
- 切分标题跟随 3 项新增测试 + document_security 回归，33 绿；全量 205 绿。

---

## W8-卡2（2026-09-26）· Agent 护栏三条 + 失败降级 + 轨迹日志

**新增了什么**
- 步数上限参数化（`AGENT_MAX_STEPS`，默认 6）；同一工具连续失败 2 次熔断停用
  （`AGENT_TOOL_FAIL_DISABLE`）并把停用告知模型；全部工具熔断=死局，直接降级。
- 失败降级：步数超限或死局 → 自动回落现有 `chat_single_turn`（Workflow）出答案，
  前缀向用户说明"本次由基础问答模式回答（原因）"——不裸奔不撒谎。
- 轨迹日志：一次执行一个 JSONL（meta/tool_call/final/degraded 逐行带时间戳）落
  `data/agent_traces/`，任何一次执行可完整回放（卡 5 前端与评测同源）。

**解决了什么问题**
- Agent 的三种死法（无限步数、死磕坏工具、死局卡住）全部有界且体面——坏情况退回
  Workflow 而非给用户空结果；出事有轨迹可查。

**验证**
- agent 测试 13 绿（新增熔断拦截/死局降级/轨迹落盘）；全量 205 绿。

---

## docs（2026-09-26）· README 呈现升级

**优化了哪些地方**：新增居中标题区（口号 + 徽章墙 + 锚点导航）、「✨ 功能亮点」速览表（8 条带数字，延迟/检索类数字按口径纪律带日期）、截图区扩为三张（补知识库管理页）、「路线图」（holdout 口径切换 / 评测集扩容+第二标注者 / 文档级权限 / 公网演示常驻）与「🙏 致谢」节。
**解决了什么问题**：原 README 数字与诚实声明齐全，但缺少"初筛 30 秒速览层"——HR/面试官扫一眼抓不到拦截率 0→100%、缓存 10ms 这些最硬的点。
**约束**：正文与全部数字未动（口径表 2026-09-24 已对齐版本），本轮仅加呈现层，无代码改动。

## W8-卡3a（2026-09-26）· 规则路由器提前落地：纯函数 + 16 题评测集 + 16 测试（接线待做）

**优化了哪些地方**
- 按 D3 拍板（纯规则起步、拿不准默认 Workflow）落地 `services/router_service.py`
  纯函数路由：高置信信号直接上表（区别/差异/哪个更/统计量词/帮我…并且…），
  弱信号用正则收紧（"哪个…好/强/适合/高"带实词校验、"多少+量词"不含
  "多少钱"），每个决策附可解释 reason（命中哪条规则的哪个词），接线后进响应 meta。
- 评测集 `eval/eval_set_agent.json` 16 题：10 道 Agent 题（对比/统计/操作复合/
  法律域/英文ID，含阶段 0"晚安焰火"原题）+ 6 道 Workflow 边界钉子题
  （"有几个技能/决斗者有哪些/多少钱"等单跳常见词钉死不上表，防将来误伤）。

**解决了什么问题**
- 卡 3 不再是纯等待：与卡 1 并行完成全部不依赖 Agent 代码的部分（三件均为
  新文件，零冲突）；卡 1 出产后卡 3 只剩"接线"临门一脚。
- 自查纠偏两处：初版"几个"会把"捷风有几个技能？"（单跳事实）误送 Agent，
  按拿不准默认 Workflow 纪律删除并以 W06 用例钉住；弱信号正则初版前缀
  `.+` 使"哪个…"开头的问句永远匹配不上，测试当场抓住修正。

**验证**
- 新增 16 用例全绿（三类路由 + 空值安全 + 评测集一致性防回归）；
  全量 pytest 199 passed（含卡 1 收口后的 agent_loop 测试，旧评测零回退）。

---

## W8-卡1（2026-09-26）· Agent 自写循环 + 检索工具化 + 重放实验

**新增了什么**
- `services/agent/`：自写 Agent 主循环（决策/终答两段式——决策轮关思考求快、终答轮开思考对齐
  Workflow 答案质量；步数上限 6 硬编码，卡 2 参数化）+ 工具注册表（`search_knowledge_base` =
  hybrid_search+rerank 薄封装，与 Workflow 同款检索质量；`execute_tool` 永不抛异常，错误以
  结构化字符串回填模型——失败模式库 ERR-3 对策）+ 受控词表（aliases.json 标准名注入 system prompt）。
- `POST /api/agent/chat`：Agent 问答端点（限流 / kb 白名单 / APIKey 防线与 Workflow 同款）；
  响应含分步轨迹 `steps`（卡 5 前端直接渲染）与来源汇总。
- `backend/scripts/w8_card1_replay.py`：可复现重放实验脚本。

**解决了什么问题**
- 补齐阶段 0 溯源缺失的"当时召回"证据：三种模式重放证实**长问题单轮检索根本召不回雷兹描述切片**
  ——「晚安焰火」幻觉定性修正为**检索+生成复合失败**（推翻条件触发）。
- Agent 四轮迭代实录（证据：工作区 `W8-AgenticRAG/evidence/卡1-重放实验.md`）：从"零检索凭记忆
  答成《英雄联盟》角色"收敛为"分步检索+捷风四技能与原文全对齐"；错误性质由"无中生有"收敛为
  "张冠李戴"，并定位其根因——**知识库切分把雷兹标题与技能表格切开，表格成了"无主切片"**
  （BM25 数据实锤）。该缺陷单轮 Workflow 与 Agent 同样受害，但 Agent 轨迹让它首次可见可定位。
- 新立卡 7（待用户拍板）：切片标题修复 + 重建索引；10 题全量基线对照顺延至修复后。

**验证**
- `tests/test_agent_loop.py` 10 个 mock 测试全绿（工具分发/异常遏制/步数超限/ToolMessage 回填/
  来源去重）；全量回归除卡 6 遗留外 104 项全过；真模型重放 4 轮，轨迹完整落 trace。
