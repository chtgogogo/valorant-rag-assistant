...
| 变体鲁棒性 8 题（D 形态：错别字/俗称/拼音/港台译名） | **100%** (8/8) | 0.854 | —（CPU 实测；GPU 推理存在浮点级微差，极端边界题可能 ±1 题） |

**带生成端到端实测**（2026-09-18，GLM-4.7-Flash，hybrid 全开）：答案关键词覆盖 **62.5%**（20/32，期望关键词来自评测集，衡量"答得全不全"），拒答正确率 **100%**（生成口径下同样全部正确拒答）。不同模型的关键词覆盖会因措辞风格不同而变化（GLM-4-Flash 时期同口径为 75.0%），对比模型时请固定评测集与判分口径。
...
- **v3.11（2026-09-23）** — LLM 调用节流提速：Critic 默认 1 轮 + 两层重试收紧，限流最坏反馈 2 分钟 → 2.3 秒。
...
- **v3.3（2026-09-20）** — GPU/CPU 混合模式（实测单条检索约 900ms → 约 110ms，约 8~10 倍；已关 TF32/flash 内核保证与 CPU 结果一致）（空闲显存检测 + OOM 优雅降级，向量与重排同进同退）、向量检索独立开关（可纯 BM25）、质量自评 Critic（灰区触发 + 换写法重检索 ≤3 轮 + 全程日志）、思考模式分级开关（改写开思考提精度，自审与最终生成关——Critic A/B 实测思考零增益）、评测脚本支持标准答案语义相似度并自动落盘报告。
