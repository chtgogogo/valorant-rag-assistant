...
- **v3.4（2026-09-20）** — 三层评测体系（L1 回归 20+8 / L2 扩充 21 题：黑话别名+多轮指代+拒答陷阱 / L3 Critic 标注集），官方数据评测题自动生成器，评测温度固定 0，报告分类型统计 + P95 延迟 + 设备口径；Critic 评审员 A/B 实测思考零增益（两版均 10/10）→ 自审默认关思考；CS2 别名陷阱题实证"提示词硬约束"防线有效。
