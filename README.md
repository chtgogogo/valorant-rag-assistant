...
| 质量自评 Critic | 重排分数处于灰区时用思考模型评估资料充分性，不足自动换角度重检索（≤3轮，全程留日志）；高分直接放行零额外延迟 |
...
- **v3.3（2026-09-20）** — GPU/CPU 混合模式（实测单条检索约 900ms → 约 110ms，约 8~10 倍；已关 TF32/flash 内核保证与 CPU 结果一致）（空闲显存检测 + OOM 优雅降级，向量与重排同进同退）、向量检索独立开关（可纯 BM25）、质量自评 Critic（灰区触发 + 换写法重检索 ≤3 轮 + 全程日志）、思考模式分级开关（改写/自评开思考、最终答案生成关）、评测脚本支持标准答案语义相似度并自动落盘报告。
