...
def ticket_list(status: str = None, limit: int = 100, _=Depends(verify_api_key)):
...
def ticket_stats(_=Depends(verify_api_key)):
    return ApiResponse(data=ticket_service.ticket_stats())


@ticket_router.post("/{ticket_id}/resolve", summary="处理工单：填标准答案，可选回流知识库")
def ticket_resolve(ticket_id: str, req: ResolveRequest, _=Depends(verify_api_key)):
...
def ticket_close(ticket_id: str, _=Depends(verify_api_key)):
    try:
