...
def main():
    ...
    try:
        client.delete_collection("valorant")
        print("    已删除旧 collection: valorant")
    except Exception:
        print("    collection 不存在或已为空，跳过")

    # 2. 清空旧文档列表
    print("[2/4] 清空旧文档列表 ...")
    doc_list = DATA_DIR / "doc_list_valorant.json"
    if doc_list.exists():
        doc_list.unlink()
    ...
    for old_file in upload_dir.glob("*.md"):
        try:
            old_file.unlink()
        except OSError:
            pass
    ...
    for src in md_files:
        ...
        try:
            chunks = upload_and_process(str(upload_dir / filename), filename, "valorant")
            total_chunks += len(chunks)
            print(f" 完成 ({len(chunks)} 块)")
        except Exception as e:
            print(f" 失败: {e}")
    print(f"\n全部完成，共 {len(md_files)} 份文档，{total_chunks} 个文本块。")
